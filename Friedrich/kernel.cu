			...
			if (layer_task->gen == gen && tasks < task_width) {
				...
				case EXECUTE_LINK:
					if (layer_task->l->counting_batch != batch) {
						...
						mutate << <layer_task->l->size / thread_num + 1, layer_task->l->size>thread_num ? thread_num : layer_task->l->size, 0, streams[tasks] >> >(layer_task->l->dev_t.t, layer_task->l->dev_t.pr, layer_task->l->dev_t.po, layer_task->l->dev_t.pr_sum, layer_task->l->dev_t.po_sum, gen);
						remove_executable(&layer_task_head, &layer_task_tail, layer_task);
						layer_task->done = true;
						tasks++;
						...
						if (layer_task->l->mutating_batch != batch) {
							if (layer_task->l->pushed_gen != gen) {
								...
								case LINK_FULL:
									...
									push_full << <s_logical->size / thread_num + 1, s_logical->size>thread_num ? thread_num : s_logical->size, 0, streams[tasks] >> > (s_phisical->dev_t[s_phisical->cur_s_dev_t].t, s_logical->offset, layer_task->l->dev_t.r, layer_task->l->dev_t.pr, layer_task->l->dev_t.pr_sum, s_logical->size, t_logical->size);
									break;
								}
								tasks++;
